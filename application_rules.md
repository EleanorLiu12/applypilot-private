...
- The body names C++ alone as a required skill, such as "must be proficient in C++" or "sound C++ fundamentals" with no other language offered. Narrowed by the user on 2026-09-21: a body that names C++ alongside any other language is no longer a skip, whether the list reads "one or more of Python, Java, C++", "Strong python, C/C++, GPU programming skills", "Proficiency in C#, C/C++, .Net", or "proficiency in C++ and Python or comparable languages". This replaces the 2026-09-13 reading, under which a conjunctive list counted as mandatory; that reading cost NVIDIA's AI Compiler Engineer New College Grad 2027, Applied Materials' 2027 New College Grad requisition and UT Austin's NSF robotics centre role in four days, and all three were reopened on 2026-09-21. Two things are unchanged: a title containing C++ is still a skip, and "C and/or C++" is read as one requirement family rather than a list of alternatives, so it still skips.
