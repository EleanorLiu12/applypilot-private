...
PROMPT="Use \$applypilot. Run one lead-finding-only job search for my existing ApplyPilot workflow. Use candidate_profile.json, application_rules.md, resume_routing.md, answer_bank.md, and dashboard/*.csv as the source of truth. Find and classify up to ${MAX_LEADS} fresh new-grad or entry-level SWE, AI engineering, ML engineering, LLM, agentic AI, or similar technical roles from public sources, including public Ashby hosted job boards and Ashby posting pages when available, and allowed LinkedIn Premium personal-search lead data when available. Do not open application flows, do not click Apply, do not create accounts, do not log in, do not submit applications, and do not answer application forms. Respect all skip rules, including sponsorship, F-1 OPT/STEM OPT, salary floor, graduation-window eligibility, citizenship/clearance, excluded companies including Stealth Startup, excluded industries, Product Manager, C++, PhD-titled jobs, and overleveled roles. Update dashboard CSV files and regenerate dashboard-ui/src/data/dashboardData.ts if leads are added. Leave application_log.csv empty unless a real application attempt occurred, which this scheduled run must not do. Stop if blocked by CAPTCHA, Cloudflare, login, 2FA, account switching, paywall, or ambiguous legal/work-authorization wording. Summarize what changed."
